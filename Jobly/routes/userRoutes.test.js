	...
	test("works for admin users", async function () {
		...
		expect(resp.body).toEqual({
			users: [
				...
				{ username: "u4", firstName: "U4F", lastName: "U4L", email: "user4@user.com", isAdmin: true }
			]
		});
	});
...
describe("PATCH /users/:username", () => {
	test("works for admin users or that user", async function () {
		const resp = await request(app)
			.patch(`/users/u2`)
			.send({
				firstName: "New"
...
describe("DELETE /users/:username", function () {
	test("works for admin users or that user", async function () {
		const resp = await request(app).delete(`/users/u2`).set("authorization", `Bearer ${adminToken}`);
...
describe("POST /users/:username/jobs/:id", function () {
	...
	test("not found if user missing", async function () {
		const resp = await request(app).post(`/users/nope/jobs/1`).set("authorization", `Bearer ${adminToken}`);
		expect(resp.statusCode).toEqual(404);
	});
